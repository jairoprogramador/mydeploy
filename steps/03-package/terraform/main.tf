...
resource "null_resource" "build_docker_image" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      docker build \
        -t ${local.container.image_uri_revision} \
  ...
  triggers = {
    ...
    code_revision = var.app_project_revision
  }
...
resource "null_resource" "push_acr_image" {
  ...
  provisioner "local-exec" {
     command = <<-EOT
      docker push ${local.container.image_uri_revision} && \
      docker push ${local.container.image_uri_latest}
    EOT
  }

  triggers = {
    //scan_result = null_resource.scan_trivy_security.id
    scan_result = null_resource.build_docker_image.id
    image_tag = var.app_project_version
    code_revision = var.app_project_revision
  }
